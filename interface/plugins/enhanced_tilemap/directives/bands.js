...
var _module = require('ui/modules').get('kibana/enhanced_tilemap');
_module.directive('bands', function () {
  function link(scope, element, attrs) {
    //colorbrewer YlOrRd-9
    var defaultColors = ["#ffffcc", "#ffeda0", "#fed976", "#feb24c", "#fd8d3c", "#fc4e2a", "#e31a1c", "#bd0026", "#800026"];
    scope.addBand = function () {
      var low = null;
      var high = null;
      if (scope.bands.length > 0) {
        var lastBand = scope.bands.slice(-1)[0];
        if (!isNaN(lastBand.high) && !isNaN(lastBand.low)) {
          low = lastBand.high;
          high = low + (lastBand.high - lastBand.low);
        }
      }
      var colorIndex = scope.bands.length;

...
define(function (require) {
  _module.directive('savedSearch', function (Private, indexPatterns) {
    var service = Private(require('ui/saved_objects/saved_object_registry')).byLoaderPropertiesName.searches;
    ...
    return {
      ...
      link: function link(scope, element, attrs) {
        ...
        function fetchSavedSearches() {
          ...
          service.find(scope.layer.filter).then(function (hits) {
            ...
            var selected = _.filter(scope.items, function (item) {
    ...
    function refreshIndexFields(indexId, callback) {
      indexPatterns.get(indexId).then(function (index) {
        var geoFields = index.fields.filter(function (field) {
          return field.type === 'geo_point' || field.type === 'geo_shape';
        }).map(function (field) {
          return field.name;
        });

        var labelFields = index.fields.filter(function (field) {
          var keep = true;
    ...
    function getIndexId(hit) {
      var state = JSON.parse(hit.kibanaSavedObjectMeta.searchSourceJSON);
      return state.index;
    }
  });
});